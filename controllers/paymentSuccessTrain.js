...
const paymentSuccessTrain = async (req, res) => {
    ...
    try {
        for (let i = 0; i < ticketIdArray.length - 1; i++) {
            ...
            const destinationNameResult = await trainPool.query(
            ...
            const trainScheduleInfo = await trainPool.query(
                getTrainScheduleInfoQuery
            );
            ...
            for (const line of textLines) {
                page.drawText(line, {
                    ...
                    font: await pdfDoc.embedFont(
                        k % 2 === 0 ? StandardFonts.HelveticaBold : StandardFonts.Helvetica
                    ),
                    color: rgb(0, 0, 0),
                });
                k++;
            ...
            for (let j = 0; j < tableHeader.length; j++) {
                ...
                page.drawRectangle({
                    x:
                        tableX +
                        colWidths.slice(0, j).reduce((acc, width) => acc + width, 0),
                    y: currentY,
                    width: colWidths[j],
                    height: rowHeight,
            ...
            const term1 =
                "How to use the PDF: This PDF serves as your valid ticket for transportation. Simply display it on your mobile device or print it out";
            const term12 =
                "and present it to the transportation staff for scanning or verification.";

            const term2 =
                "Do not share with others: Please keep this ticket for your personal use only. Sharing it with others may result in loss of access ";
            const term21 = "to the transportation service.";

            const term3 =
                "Lost ticket: If you happen to lose this PDF ticket, please contact our customer support immediately at triptix.sfz@gmail.com";
            const term32 =
                "to report the loss and request assistance. We will do our best to assist you in resolving the issue promptly.";
...
const paymentSuccessTrainProfile = async (req, res) => {
    trainPool.query("BEGIN");

    const data = req.body;
    const { trainScheduleId, ticketId } = req.body;
    const ssl = new SSLCommerzPayment(store_id, store_passwd, is_live);
    const validation = ssl.validate(data);
    validation
        .then((validation) => {
    ...
    try {
        ...
        for (let j = 0; j < tableHeader.length; j++) {
            ...
            page.drawRectangle({
                x:
                    tableX + colWidths.slice(0, j).reduce((acc, width) => acc + width, 0),
                y: currentY,
                width: colWidths[j],
                height: rowHeight,
                color: tableHeaderBackgroundColor,
