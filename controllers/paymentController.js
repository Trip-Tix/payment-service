...
//sslcommerz success
const paymentSuccessProfile = async (req, res) => {

    busPool.query('BEGIN');

    const data = req.body;
    const { busScheduleId, ticketId } = req.body;
    const ssl = new SSLCommerzPayment(store_id, store_passwd, is_live)
    const validation = ssl.validate(data);
    validation.then(validation => {
        console.log('Validation success');
    ...
    const transactionId = data.tran_id;
    const paymentMedium = data.card_issuer;
    console.log(transactionId, paymentMedium, ticketId, busScheduleId)
    try {
